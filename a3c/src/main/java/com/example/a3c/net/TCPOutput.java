...
import com.example.a3c.model.TCB;
import com.example.a3c.model.TCB.TCBStatus;
import com.example.a3c.model.Packet.TCPHeader;
...
public class TCPOutput implements Runnable
{
            ...
            throws IOException
    {
        ...
        if (tcpHeader.isSYN())
        {
            SocketChannel outputChannel = SocketChannel.open(); /* 内部已经关联了一个底层的套接字 */
            ...
            try
            {
                outputChannel.connect(new InetSocketAddress(destinationAddress, destinationPort));/* 先protect再connect？ */
                if (outputChannel.finishConnect()) /* 非阻塞模式下，connect直接返回结果，所以需要判断 */
                {
